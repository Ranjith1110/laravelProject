...
class AdmissionController extends Controller
{
    ...
    public function admissionEnquiryList(){
        if(session()->has('mamce_admin')){
            $admissionEnquiry = AdmissionEnquiry::orderBy('id','desc')->paginate();
            return view('dashboard.admission.list',compact('admissionEnquiry'));
        }else{
            return redirect()->route('index');
        }
    }
    public function admissionEnquiryView($id){
